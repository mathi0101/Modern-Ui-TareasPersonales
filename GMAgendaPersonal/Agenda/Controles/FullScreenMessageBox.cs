...
namespace Agenda.Controles {
	...
	public class FullScreenMessageBox : MetroSetForm {
		...
		/// The Constructor.
		/// </summary>
		private FullScreenMessageBox() {
			base.Font = Fonts.Regular(10.5f);
			ShowInTaskbar = false;
		...
		public static DialogResult Show(Form form, string content, string caption, MessageBoxButtons buttons, MessageBoxIcon icon) {
			...
			var msgBox = new FullScreenMessageBox {
				OwnerForm = form ?? throw new ArgumentNullException(message),
				Content = content,
				Caption = caption,
				Buttons = buttons,
				Size = new Size(form.Width - 15, (form.Height / 3) - 1),
